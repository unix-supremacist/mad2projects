...
void initiate_delayed_warp(void) {
    ...
    if (sDelayedWarpOp != WARP_OP_NONE && --sDelayedWarpTimer == 0) {
        ...
        if (gChallengeLevel != 0) {
            if (sDelayedWarpOp == WARP_OP_STAR_EXIT || sDelayedWarpOp == WARP_OP_CREDITS_START) {
                extern void exit(int status);
                exit(0);
            }
            if (sDelayedWarpOp == WARP_OP_DEATH || sDelayedWarpOp == WARP_OP_WARP_FLOOR || sDelayedWarpOp == WARP_OP_GAME_OVER) {
                gMarioState->health = 0x880; // Full health (8 wedges)
                gMarioState->numLives = 4;   // Reset lives count to prevent game over loop
                extern void reset_camera(struct Camera *c);
                extern struct Camera *gCamera;
                if (gCamera) {
                    reset_camera(gCamera);
                }
                initiate_warp(gChallengeLevel, 1, 0x0A, WARP_FLAGS_NONE);
...
s32 play_mode_paused(void) {
    if (gMenuOptSelectIndex == MENU_OPT_NONE) {
        ...
        if (gChallengeLevel != 0) {
            // Do not allow exiting course in challenge mode! Just resume normal play.
            raise_background_noise(1);
            gCameraMovementFlags &= ~CAM_MOVE_PAUSE_SCREEN;
            set_play_mode(PLAY_MODE_NORMAL);
...
s32 init_level(void) {
    ...
    if (gCLIOpts.LevelNumOverride) {
        extern s16 gChallengeLevel;
        gChallengeLevel = gCLIOpts.LevelNumOverride;
        // Keep the intro scene skip flag!
        gGlobalGameSkips = (gGlobalGameSkips & ~GAME_SKIP_GENERAL) | GAME_SKIP_INTRO_SCENE;
