...
static struct ControllerAPI *controller_implementations[] = {
    ...
    &controller_sdl,
    // Must come after &controller_sdl -- mad2modloader's injected input
    // (see controller_udp.c) overwrites whatever the real gamepad backend
    // above just wrote into `pad` this frame when a fresh packet is
    // available, and falls back to leaving it untouched otherwise.
    &controller_udp,
    #endif
    &controller_keyboard,
