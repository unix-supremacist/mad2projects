...
#endif
{
    ...
    while (true) {
        ...
        // Poll events inside the crash handler loop so users can close/exit
        #if defined(WAPI_SDL2)
        SDL_Event event;
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT || 
                (event.type == SDL_KEYDOWN && (event.key.keysym.sym == SDLK_ESCAPE || event.key.keysym.sym == SDLK_RETURN || event.key.keysym.sym == SDLK_SPACE)) ||
